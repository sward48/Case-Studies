...
xr <- range(df_map2024$lon, na.rm = TRUE)
yr <- range(df_map2024$lat, na.rm = TRUE)
leaf <- leaf %>% fitBounds(lng1 = xr[1], lat1 = yr[1], lng2 = xr[2], lat2 = yr[2])

leaf
#htmlwidgets::saveWidget(leaf, "hamden2024.html", selfcontained = TRUE)
```
...
# ---- fit to bounds (no sf needed) ----
xr <- range(df_map2023$lon, na.rm = TRUE)
yr <- range(df_map2023$lat, na.rm = TRUE)
leaf <- leaf %>% fitBounds(lng1 = xr[1], lat1 = yr[1], lng2 = xr[2], lat2 = yr[2])

leaf
htmlwidgets::saveWidget(leaf, "hamden2023.html", selfcontained = TRUE)
```
